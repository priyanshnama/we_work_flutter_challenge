...
class TopRatedMoviesBloc extends Bloc<TopRatedMoviesEvent, TopRatedMoviesState> {
  ...
  TopRatedMoviesBloc(this.weMoviesRepository) : super(TopRatedMoviesInitial()) {
    on<FetchTopRatedMoviesEvent>((event, emit) async {
      ...
      try {
        currentPage = 1;
        topRatedMovies = await weMoviesRepository.getTopRatedMovies(page: currentPage);
    ...
    on<FetchMoreTopRatedMoviesEvent>((event, emit) async {
      try {
        currentPage++;
        final moreMovies = await weMoviesRepository.getTopRatedMovies(page: currentPage);
